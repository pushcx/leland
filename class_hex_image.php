<?
/*
This code should probably be refactored to not require that external code call
some functions before or after others, but it's a pretty quick cleanup and OO
conversion of the old procedural code. The project I was going to use this for
got shifted to perpetual backburner, so I'm in no real hurry.

Todo:
 * method for setting the line color/weight/transparency settings
 * method for drawing straight arrows from one hex to another
 * method for drawing arcing arrows from one hex to another
 * method for having a background image and, optionally, transparent hexes
 * method for adding background images to tiles
 * make text() clip the text to the width of the hex
 * probably clean up the switch statements in neighbor() and wall() to use arrays
 * support both NS (current) and EW orientation for hexes
 * remove start() method, allow methods to be called in any order
 * add a coords class?
 * add hex and hexfield classes and make this aggregate them?
 * all-around better error and input checking, is currently too trusting
 * catch errors and print them in an image to save debugging hassle
...
class Hex_image {
	...
	function Hex_image() {		// constructor
	...
	function set_shape($type, $shape = array()) {
		switch($type) {
		case "rectangle":	// ignores shape, uses x and y
...
// internals
	function drawshape() {
		for($x = 1; $x <= $this->xsize; $x++)
			for($y = 1; $y <= $this->ysize; $y++)
				if ($this->shape[$x][$y])
					$this->drawhex($x, $y, $this->shape[$x][$y], 'gray');
	}
